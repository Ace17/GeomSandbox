...
struct SplitPolygonAlgorithm
{
  ...
  static std::vector<std::vector<Vec2>> execute(Input input)
  {
    ...
    // classify points
    for(auto& pos : input.polygon)
    {
      const float epsilon = 0.001;
      const float x = dotProduct(pos, input.normal) - input.dist;
      int side;
      if(x > +epsilon)
        side = +1;
      else if(x < -epsilon)
    ...
    // insert the intersection points
    {
      ...
      for(int curr = 0; curr < N; ++curr)
      {
        ...
        if(polygon[curr].side * polygon[next].side == -1)
        {
          ...
          const float q0 = dotProduct(p0, input.normal);
          const float q1 = dotProduct(p1, input.normal);
          const float ratio = (input.dist - q0) / (q1 - q0);
    ...
    {
      const Vec2 tangent = -rotateLeft(input.normal);
  ...
  static std::vector<std::vector<Vec2>> cutAlongDiagonals(span<PointWithSide> polygon, span<Diagonal> diagonals)
  {
    ...
    auto byIncreasingArcLength = [N](Diagonal p, Diagonal q)
    { return arcLength(N, p.a, p.b) < arcLength(N, q.a, q.b); };
